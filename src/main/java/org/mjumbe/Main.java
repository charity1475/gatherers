...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			logger.info("Starting  {} scrape job at {} ...", config.getDecoderName(), System.currentTimeMillis());
			DataFetcher dataFetcher = new DataFetcher();

			try (DataCache dataCache = new DataCache(config.getRedisHost(), config.getRedisPort(), config.getRedisPassword())) {
				String fetchedData = dataFetcher.fetchData(config.getHttpRequestUrl());
		...
		logger.info("Decoder scrape job completed successfully at {} ...", System.currentTimeMillis());
	}
